...
export function buildNightSteps(activeRoleIds, playMode = 'local') {
  ...
  steps.push({
    key: 'evil_open',
    text: isOnline ? '坏人确认队友。' : `${evilLabel}，请睁开双眼，互相确认身份。`,
    audio: '/audio/evil_open.mp3',
    countdown: 5,
  })
  steps.push({
    key: 'evil_close',
    text: isOnline ? '坏人等待。' : `${evilLabel}，请闭眼。`,
  ...
  steps.push({
    key: 'merlin_open',
    text: isOnline ? `梅林确认${merlinSee}。` : `梅林请睁眼，${merlinSee}，请伸出大拇指。`,
    audio: '/audio/merlin_open.mp3',
    countdown: 5,
  })
  steps.push({
    key: 'merlin_close',
    text: isOnline ? '梅林等待。' : '梅林请闭眼，坏人请放下大拇指。',
  ...
  if (has('percival')) {
    ...
    steps.push({
      key: 'percival_open',
      text: isOnline ? `派西维尔确认${pSee}。` : `派西维尔请睁眼，${pSee}，请伸出大拇指。`,
      audio: '/audio/percival_open.mp3',
      countdown: 5,
    })
    steps.push({
      key: 'percival_close',
      text: isOnline ? '派西维尔等待。' : '派西维尔请闭眼，请放下大拇指。',
